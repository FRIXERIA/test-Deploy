  ...
  if (res == 401) {
    ...
    //edit
      ...
      if (data.value.publishDate !== null) {
        datePublish.value = new Date(data.value.publishDate)
        ...
        timePublish.value = `${hourPublish
          .toString()
          .padStart(2, "0")}:${minutePublish.toString().padStart(2, "0")}`;
      ...
      if (data.value.closeDate !== null) {
        dateClose.value = new Date(data.value.closeDate)
          .toLocaleString("ko-KR", {
            year: "numeric",
            month: "2-digit",
            day: "2-digit",
        ...
        timeClose.value = `${hourClose
          .toString()
          .padStart(2, "0")}:${minuteClose.toString().padStart(2, "0")}`;
...
onBeforeUpdate(() => {
  ...
  if (params.id === undefined) {
    ...
    if (
      data.value.announcementCategory === undefined ||
      data.value.announcementCategory === ""
    ) {
      useButton.value = true;
    }
    if (datePublish.value) {
      if (!timePublish.value) timePublish.value = `06:00`;
    }
    if (dateClose.value) {
      if (!timeClose.value) timeClose.value = `18:00`;
    }
  ...
  if (filesList.value?.length < 5) {
    attachBtn.value = false;
  }
  ...
  if (fileData.value !== undefined && nameData.value !== undefined) {
    for (let i = 0; i < fileData.value.length; i++) {
      ...
      selectedFiles.value.push(
        new File([fileData.value[i]], nameData.value[i].fileName)
      );
    }
  }
...
let update = async (edit) => {
  ...
  try {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_ROOT_API}/api/announcements/${edit.id}`,
      {
        method: "PUT",
        headers: {
          "content-type": "application/json",
...
const isFileAlreadySelected = (file) => {
      return selectedFiles.value.some((selectedFile) => selectedFile.name === file.name);
};


const isNameAlreadySelected = (file) => {
    if (nameData.value!==null || nameData.value!==undefined) {
        let status = nameData.value.some((selectedFile) => {
            return selectedFile.fileName === file.name;
        });
        return status; // Returning the 'status' variable
    } else {
        return; // Or return any appropriate value if nameData.value is falsy
    }
};

const handleFileChange = (event) => {
  const files = event.target.files;
  filesList.value = Array.from(fileInput.value.files);
  for (let i = 0; i < files.length; i++) {

    const file = files[i];
    if(params.id){
    if(nameData.value){
    if (!isNameAlreadySelected(file)) {
...
//   filesList.value = Array.from(fileInput.value.files);
let deleted = filesList.value.length - 5
let i =0 
  ...
  <div class="flex flex-col ml-16 mr-16">
    ...
    <div class="flex flex-row">
      <div class="font-medium">
        Title<span class="text-red-600 pr-96 mr-36 pl-1">*</span>
      </div>
      <p class="text-red-600 font-semibold ml-96 pl-80" v-show="noTiltle">
        {{ notEmpty }}
      </p>
      <p class="text-red-600 font-semibold ml-96 pl-80" v-show="limitTitle">
        {{ titleLimit }}
      </p>
      <p class="text-red-600 font-semibold ml-96 pl-80" v-show="editTitle">
        {{ notEditTitle }}
      </p>
    </div>
    <div class="pt-1">
      <input
        class="ann-title w-full border border-teal-500 rounded-lg"
        v-model="data.announcementTitle"
        type="text"
        maxlength="200"
    ...
    <div class="pt-3 font-medium">
      Category<span class="text-red-600 pr-72 pl-1">*</span>
    </div>
    <div class="pt-1" id="selectCategory">
      ...
      >
        <option v-for="(categorys, index) in category" :key="index">
    ...
    <div class="flex flex-row">
      <div class="pt-3 font-medium">
        Description<span class="text-red-600 pr-96 mr-96 pl-1">*</span>
      </div>
      <p class="text-red-600 font-semibold ml-96 pl-8 pt-3" v-show="noDesc">
        {{ notEmptyDesc }}
      </p>
      <p class="text-red-600 font-semibold ml-96 pl-8 pt-3" v-show="limitDesc">
        {{ descLimit }}
      </p>
    </div>
    <div class="h-48 pt-1">
      <QuillEditor
        ...
        them="snow"
        toolbar="full"
        v-model:content="data.announcementDescription"
        contentType="html"
    ...
    <form id="fileForm">
      ...
      <div class="flex flex-col">
        <p
          v-if="nameData.length !== 0"
          v-for="(file, index) in nameData"
          :key="index"
        ...
        >
          ...
          <button
            type="button"
            @click="deleteFile(index, file.fileName, params.id)"
            class="pl-3"
          >
            <svg
              ...
              stroke-width="1.5"
              stroke="currentColor"
              class="w-6 h-6 stroke-red-500 hover:stroke-red-900"
            >
              <path
      ...
      <div v-show="fileLimit" class="text-red-500">
        The file size cannot be larger than 20 MB.
      </div>
      <div
        v-show="selectedFiles.length >= 5 || nameData.length >= 5"
        class="text-red-500"
      >
        The announcement can have at most 5 attachments.
      </div>
    </form>
    <p>{{ msg }}</p>
    ...
    <div class="flex flex-row">
      <input
        class="ann-publish-date w-40 border border-teal-500 rounded-lg"
        type="date"
        @change="changeValue"
        id="publish"
        v-model="datePublish"
      />
      <input
        class="ann-publish-time ml-3 w-40 border border-teal-500 rounded-lg"
        type="time"
        v-model="timePublish"
    ...
    <div class="flex flex-row">
      <input
        class="ann-close-date w-40 border border-teal-500 rounded-lg"
        type="date"
        @change="changeValue"
        id="close"
        v-model="dateClose"
      />
      <input
        class="ann-close-time ml-3 w-40 border border-teal-500 rounded-lg"
        type="time"
        v-model="timeClose"
        :disabled="!dateClose"
      />
    ...
    <div>
      <input
        class="ann-display mr-1 border border-teal-500 rounded-lg"
        type="checkbox"
        ref="check"
        v-model="data.announcementDisplay"
    ...
    <div class="pt-5 flex flex-row">
      ...
      <button
        type="submit"
        class="btn btn-outline btn-info ann-button"
        id="submit"
        v-if="params.id"
        @click="update(data)"
        :disabled="useButton"
      ...
      <button
        type="submit"
        class="btn btn-outline btn-success ann-button"
        id="submit"
        @click="addNew(data)"
        :disabled="useButton"
        v-if="!params.id"
      ...
      <button
        type="button"
        class="btn btn-outline btn-error ann-button ml-2"
        @click="cancelAction"
      >
        Cancel
      </button>
